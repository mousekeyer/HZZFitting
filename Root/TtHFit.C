...
TtHFit::TtHFit(string name){
    ...
    TtHFitter::IMAGEFORMAT.push_back("eps");
    TtHFitter::IMAGEFORMAT.push_back("pdf");
    
    //
    fSystematics.clear();
    fSystematicNames.clear();
    fNSyst = 0;
    fSystConstrainType.clear();
    //
    fNormFactors.clear();
...
void TtHFit::ReadConfigFile(string fileName,string options){
    ...
    if (cs) {
        ...
        param = cs->Get("NScanPoints"); if( param != "" ){fNScanPoints = atof(param.c_str()); }
        param = cs->Get("POIScanMin"); if( param != "" ){fPOIScanMin= atof(param.c_str()); }
        param = cs->Get("POIScanMax"); if( param != "" ){fPOIScanMax= atof(param.c_str()); }

        param = cs->Get("DoLimitMacro");    if( param != "" ){
            std::transform(param.begin(), param.end(), param.begin(), ::toupper);
            if( param == "TRUE" ){
                fDoLimitMacro= true;
            } else if ( param == "FALSE" ){
    ...
    while(true){
        ...
        param = cs->Get("LogScaleX"); if( param != "" ){
            std::transform(param.begin(), param.end(), param.begin(), ::toupper);
            if(param=="TRUE") reg->fLogScaleX = true;
            if(param=="FALSE") reg->fLogScaleX = false;
        }
...
//     Sample *sam;
    ...
    while(true){
        ...
        string constraintype= cs->Get("ConstrainType");
...
//         // ...
        ...
        if ( decorrelate == "" ) {
            ...
            if(constraintype=="")
            {
              fSystConstrainType.push_back("GAUSSIAN");
            }
            else
            {
              fSystConstrainType.push_back(constraintype);
            }
...
void TtHFit::DrawAndSaveAll(string opt){
    ...
    for(int i_ch=0;i_ch<fNRegions;i_ch++){
        ...
        if(isPostFit){
            ...
            p->SaveAs(     (fName+"/Plots/"+fRegions[i_ch]->fName+"_postFit_"+fName+"."+TtHFitter::IMAGEFORMAT[i_format] ).c_str());
        }
        else{
            ...
            for(int i_format=0;i_format<(int)TtHFitter::IMAGEFORMAT.size();i_format++)
                p->SaveAs(     (fName+"/Plots/"+fRegions[i_ch]->fName+"_"+fName+"."+TtHFitter::IMAGEFORMAT[i_format] ).c_str()); 
        }
    }
}
...
void TtHFit::ToRooStat(bool makeWorkspace, bool exportOnly){
    ...
    for(int i_syst=0;i_syst<fNSyst;i_syst++){
        ...
        std::cout<< "DEBUG: ConstrainType -> " <<  fSystConstrainType[i_syst] << std::endl;

        if(fSystConstrainType[i_syst]=="GAMMA") 
        {
          std::cout<< "DEBUG: Set GAMMA constrain"  << std::endl;
          meas.AddGammaSyst(fSystematics[i_syst]->fName.c_str(), 1.);
        }
        if(fSystConstrainType[i_syst]=="LOGNORM") 
        {
          std::cout<< "DEBUG: Set LOGNORM constrain"  << std::endl;
          meas.AddLogNormSyst(fSystematics[i_syst]->fName.c_str(), 1.);
        }
    }
    //
...
void TtHFit::PlotFittedNP(){
    ...
    if(fFitResults){
        ...
        for(int i_format=0;i_format<(int)TtHFitter::IMAGEFORMAT.size();i_format++)
            fFitResults->DrawPulls(fName+"/NuisPar_"+fName+fSuffix+"."+TtHFitter::IMAGEFORMAT[i_format],"all");
...
void TtHFit::PlotCorrelationMatrix(){
    ...
    if(fFitResults){
        for(int i_format=0;i_format<(int)TtHFitter::IMAGEFORMAT.size();i_format++)
            fFitResults->DrawCorrelationMatrix(fName+"/CorrMatrix"+fSuffix+"_"+fName+"."+TtHFitter::IMAGEFORMAT[i_format],TtHFitter::CORRELATIONTHRESHOLD);
    }
}
...
void TtHFit::GetLimit(){
    ...
    string cmd;
    std::vector<string> v_cmd;
    v_cmd.clear();

    RooStats::HypoTestInverterResult *r=0;
    
    RooStats::HypoTestTool calc;
    ...
    if(fWorkspaceFileName!=""){
        ...
        // Add
        TFile fWorkspacefile(fWorkspaceFileName.c_str()); 

        RooWorkspace *w=(RooWorkspace*)fWorkspacefile.Get("combined");

        const char *nuisPrior=0;
        ...
        if(!fDoLimitMacro)
        {
          r=calc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataName.c_str(), 2 , 3 , true, fNScanPoints, fPOIScanMin, fPOIScanMax, 1000, false, nuisPrior);
        }
...
//        const char * nuisPriorName )
        ...
        if(!fDoLimitMacro)
        {
          TFile fWorkspacefile(outputName); 

          RooWorkspace *w=(RooWorkspace*)fWorkspacefile.Get("combined");

          const char *nuisPrior=0;
          const char *dataname="ttHFitterData";
          ...
          //        r=calc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataname, 2 , 3 , true, fNScanPoints, fPOIScanMin, fPOIScanMax, 1000, false, nuisPrior);
          /// here we go ...

          bool m_useScanRange=true;
          if(fPOIScanMax<fPOIScanMin) m_useScanRange=false;
          ...
          if (!m_useScanRange) {
            ...
            r=calc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataname, 2 , 3 , true, fNScanPoints, 0, -1, 1000, false, nuisPrior);
          }

          /// then reevaluate with proper settings
          if ( r!=0 || m_useScanRange) {
            double minRange = -1;
            double maxRange = -1;
            if (m_useScanRange) { // next fit is done between the user defined values
              minRange = fPOIScanMin;
              maxRange = fPOIScanMax;
            }
            else { // next fit is done between 0 and the expected upper limit + 2 sigma
              (void) r->ExclusionCleanup();             

              int i=0; 
              while (i<5) {
                if (min_CLs(r)>0.05) { 
                  r= RedoScan(w,r, calc);
                  if ( r!=0 ) { (void) r->ExclusionCleanup(); }
                }
                else { break; }
                i+=1;
              }
              minRange = 0;
              maxRange = 1.10 * r->GetExpectedUpperLimit(2);
              delete r; r=0;
            }

            //  hypo = RooStats::DoHypoTestInversion(w, m_nToys, m_calcType, m_testStatType, m_useCLs, m_nPoints, minRange, maxRange);

            //          int npoint=int(maxRange/0.2);
            r=calc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataname, 2 , 3 , true, fNScanPoints, minRange, maxRange, 1000, false, nuisPrior);

            //          int nPointsRemoved = r->ExclusionCleanup();

            if (m_useScanRange) {
              if ((1.2 * r->GetExpectedUpperLimit(2)) > fPOIScanMax) {
                std::cout << "WARNING:  Scan Range is too near to the upper limit + 2 sigma! (calc exp limit + 2 sigma: " << r->GetExpectedUpperLimit(2)
                  << ", set range: " << fPOIScanMax<< ")" << std::endl;
              }
            }
          }       
...
//        for(unsigned cmdi=0; cmdi<99; cmdi++)
//        {
//          stringstream ss;
//          ss<< 0.01*(cmdi+1);
...
//          cmd = "root -l -b -q 'runAsymptoticsCLs.C+(\""+(string)outputName+"\",\"combined\",\"ModelConfig\",\"ttHFitterData\",\"asimovData_0\",\"./"+fName+"/Limits/\",\""+fName+fSuffix+"\","+ss.str()+")'";

//          if(fDoLimitMacro) gSystem->Exec(cmd.c_str());
//        }
    }
    
    if(!fDoLimitMacro)
    {
      std::string outputprefix="./"+fName+"/Limits/";
      std::string outputlimitfilename=fName+fSuffix+".root";

      system(("mkdir -vp " + outputprefix).c_str());

      std::cout <<"####### Limit Summary ########" <<std::endl;
      std::cout <<"Observed      : " << r->UpperLimit() <<std::endl;
      std::cout <<"Expected Med  : " << r->GetExpectedUpperLimit() <<std::endl;
      std::cout <<"Expected +2   : " << r->GetExpectedUpperLimit(2) <<std::endl;
      std::cout <<"Expected +1   : " << r->GetExpectedUpperLimit(1) <<std::endl;
      std::cout <<"Expected -1   : " << r->GetExpectedUpperLimit(-1) <<std::endl;
      std::cout <<"Expected -2   : " << r->GetExpectedUpperLimit(-2) <<std::endl;

      TFile *outlimitfile=new TFile((outputprefix+outputlimitfilename).c_str(), "recreate");

      TH1F *h_lim=new TH1F("limit", "limit", 7, 0 ,7);
      ...
      h_lim->SetBinContent(1, r->UpperLimit() );
      h_lim->SetBinContent(2, r->GetExpectedUpperLimit() );
      h_lim->SetBinContent(3, r->GetExpectedUpperLimit(2) );
      h_lim->SetBinContent(4, r->GetExpectedUpperLimit(1) );
      h_lim->SetBinContent(5, r->GetExpectedUpperLimit(-1) );
      h_lim->SetBinContent(6, r->GetExpectedUpperLimit(-2) );
      h_lim->SetBinContent(7, 0 );
      h_lim->Write();
      outlimitfile->Close();

      const char *outFileprefix=(outputprefix+"OutCLscan").c_str();
      const char *outFiletype=".pdf";

      calc.AnalyzeResult(r, 2, 3, true, fNScanPoints, outFileprefix, outFiletype);
    }
    else
    {
...
RooStats::HypoTestInverterResult* TtHFit::RedoScan(RooWorkspace* w, RooStats::HypoTestInverterResult* hypo, RooStats::HypoTestTool &hypoCalc ) {
...
//    hypo = RooStats::DoHypoTestInversion(w,1,2,m_testStatType,true,20,0.,newX);
    ...
    hypo=hypoCalc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataname, 2 , 3 , true, fNScanPoints, 0., newX, 1000, false, nuisPrior);
...
double TtHFit::min_CLs(RooStats::HypoTestInverterResult* hypo) {
    double CLs=1.;
...
void TtHFit::GetSignificance(){
 ...
 //       if(!fDoLimitMacro)
   ...
   //       double muVal = 1.0;
           ...
           RooStats::HypoTestInverterResult* result=calc.RunHypoTestInverter(w, ModelSBname, ModelBname, dataname, 2 , 3 , true, 1, 1, 1, 1000, false, nuisPrior);

       //   if (result==0) { return lres; }    
          lres = get_Pvalue( result );

          std::string outputprefix="./"+fName+"/Significance/";
          std::string outputsigfilename="CLs_"+fName+fSuffix+".root";

          system(("mkdir -vp " + outputprefix).c_str());

          std::cout <<"####### CLs Summary ########" <<std::endl;
          std::cout <<"Observed      : " << lres.GetCLs()      <<std::endl;
          std::cout <<"Expected Med  : " << lres.GetCLsexp()   <<std::endl;
          std::cout <<"Expected +2   : " << lres.GetCLsu2S()  <<std::endl;
          std::cout <<"Expected +1   : " << lres.GetCLsu1S()  <<std::endl;
          std::cout <<"Expected -1   : " << lres.GetCLsd1S() <<std::endl;
          std::cout <<"Expected -2   : " << lres.GetCLsd2S() <<std::endl;

          TFile *outsigfile=new TFile((outputprefix+outputsigfilename).c_str(), "recreate");

          TH1F *h_sig=new TH1F("CLs", "CLs", 7, 0 ,7);

          h_sig->GetXaxis()->SetBinLabel(1, "Observed");
          h_sig->GetXaxis()->SetBinLabel(2, "Expected");
          h_sig->GetXaxis()->SetBinLabel(3, "+2sigma");
          h_sig->GetXaxis()->SetBinLabel(4, "+1sigma");
          h_sig->GetXaxis()->SetBinLabel(5, "-1sigma");
          h_sig->GetXaxis()->SetBinLabel(6, "-2sigma");
          h_sig->GetXaxis()->SetBinLabel(7, "Global status"); // do something with this later

          h_sig->SetBinContent(1, lres.GetCLs()       );
          h_sig->SetBinContent(2, lres.GetCLsexp()    );
          h_sig->SetBinContent(3, lres.GetCLsu2S()   );
          h_sig->SetBinContent(4, lres.GetCLsu1S()   );
          h_sig->SetBinContent(5, lres.GetCLsd1S()  );
          h_sig->SetBinContent(6, lres.GetCLsd2S()  );
          h_sig->SetBinContent(7, 0 );
          h_sig->Write();
          outsigfile->Close();
...
void TtHFit::ProduceNPRanking( string NPnames/*="all"*/ ){
    ...
    for(int i_syst=0;i_syst<fNSyst;i_syst++){
        if(NPnames=="all" || NPnames==fSystematics[i_syst]->fName || 
          ...
          if ((fSystematics[i_syst]->fName).find("MCStat")==std::string::npos)
          {
...
//     cmd       += " cat "+fName+"/Fits/NPRanking"+fLoadSuf+"_* > "+fileToRead+" ; ";
    ...
    while (!fin.eof()){
      ...
      for(unsigned int parai=0; parai< parname.size(); parai++)
      {
        if(parname.at(parai)==paramname)
        {
          found_dup=true;
        }
      }

      std::cout<< "NpName: " << paramname.c_str() <<std::endl;
      if(found_dup==false)
      {
    ...
    TCanvas *c = new TCanvas("c","c",1000,newHeight);
    c->SetTicks(0,0);
    ...
    ATLASLabelNew(0.52,(1.*(offsetDown+offsetDown1+SIZE*lineHeight+0.55*offsetUp1)/newHeight), (char*)"Internal", kBlack, gStyle->GetTextSize());
    myText(       0.52,(1.*(offsetDown+offsetDown1+SIZE*lineHeight+0.3*offsetUp1)/newHeight), 1,Form("#sqrt{s} = %s, %s",fCmeLabel.c_str(),fLumiLabel.c_str()));
    
    gPad->RedrawAxis();
    
    for(int i_format=0;i_format<(int)TtHFitter::IMAGEFORMAT.size();i_format++)
        c->SaveAs( (fName+"/Ranking_"+fName+fSuffix+"."+TtHFitter::IMAGEFORMAT[i_format]).c_str() );
...
void TtHFit::GetLikelihoodScan( RooWorkspace *ws, string varName, RooDataSet* data){
  ...
  TF1* poly = new TF1("poly2","[0]+[1]*x+[2]*x*x",-10,10);
  ...
  RooPlot* frameLH = var->frame(Title("-log(L) vs "+vname),Bins(100),Range(-0.5,2.5));
  ...
  can->SaveAs( fName+"/"+LHDir+"NLLscan_"+varName+".eps");
  can->SaveAs( fName+"/"+LHDir+"NLLscan_"+varName+".pdf");
}
...
LimitResult TtHFit::get_Pvalue( const RooStats::HypoTestInverterResult* fResults, bool doUL ) 
{
    ...
    /// 2. discovery case
        ...
        if (resultIsAsymptotic) {
          ...
          RooStats::SamplingDistribution * s = fResults->GetExpectedPValueDist(0) ;
            if (!s) { 
                return dummyResult;
                //exit(1); 
            } 
            ...
            RooStats::SamplingDistribution* t = oneresult->GetAltDistribution() ;
