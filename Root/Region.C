...
TthPlot* Region::DrawPreFit(string opt){
    ...
    if(TtHFitter::OPTION["FourTopStyle"]>0){
        ...
        p->AddLabel(Form("#font[52]{#mu=%.2f}", 1));
...
//        p->AddLabel("Post-Fit, #mu=1");
    ...
    for(int i=0;i<fNSamples;i++){
        ...
        for(int i_norm=0;i_norm<fSampleHists[i]->fNNorm;i_norm++){
            ...
            {
              nfValue = nf->fNominal;
              nfValue_down = nf->fNominal;
              nfValue_up = nf->fNominal;
            }
            else 
            {
              nfValue = fitRes->GetNuisParValue(nfName);
              nfValue_up = fitRes->GetNuisParErrUp(nfName);
              nfValue_down = fitRes->GetNuisParErrDown(nfName);
            }
...
//             if(nfName=="SigXsecOverSM" && nfValue==0) nfValue = 1;   // FIXME
            if(nfName=="mu") 
            {
             // nfValue = 1;   // FIXME
              muvalue=nfValue;
              muvalue_up=nfValue_up;
              muvalue_down=nfValue_down;
            }
            hSmpNew[i]->Scale(nfValue);
        }
    }
    
    p->AddLabel(Form("Post-Fit, #mu=%.2f^{+%.2f}_{%.2f}", muvalue, muvalue_up, muvalue_down));
    //
