...
void CorrelationMatrix::Draw(string path, const double minCorr){
    ...
    // 0.5) Skip some NPs
    //
    string npToExclude[] = {"gamma_","MCStat"};
//    string npToExclude[] = {"gamma_"};
    bool skip = false;
...
//             h_corr -> SetBinContent(N-jNP,iNP+1,100.*GetCorrelation(iSystName, jSystName));
    ...
    gStyle->SetPalette(1);
    h_corr->SetMarkerSize(0.85*1000);
    gStyle->SetPaintTextFormat(".1f");
    gPad->SetLeftMargin(0.8*600/(size+300));
    gPad->SetBottomMargin(0.8*600/(size+300));
...
//     gPad->SetTopMargin(0.1*600/(size+100));
    ...
    h_corr->GetXaxis()->SetLabelSize( h_corr->GetXaxis()->GetLabelSize()*0.85 );
    h_corr->GetYaxis()->SetLabelSize( h_corr->GetYaxis()->GetLabelSize()*0.85 );
    c1->SetTickx(0);
    c1->SetTicky(0);
