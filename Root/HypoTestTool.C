...
//    plot->Draw("CLb 2CL");  // plot all and Clb
    ...
    TString ULfilenameeps = TString("upperlimit_cls_poi_") + gSystem->BaseName(mResultFileName) + ".eps";
    TString ULfilenameroot = TString("upperlimit_cls_poi_") + gSystem->BaseName(mResultFileName) + ".root";
   // m_logger << kINFO << " writing result plot to " << TString(outputDir+ULfilename).Data() << GEndl; 
    ...
    c_sig->SaveAs( TString(outputDir+ULfilenameeps).Data() );
    c_sig->SaveAs( TString(outputDir+ULfilenameroot).Data() );
...
//    else 
    ...
    if (!bModel || bModel == sbModel) {
        ...
        if(!bModel->GetParametersOfInterest()->first()){
            std::cout <<  "HypoTestTool: sbModel has no POI!" << std::endl;
            return false;
        }
        ...
        if (!var){
          ...
          std::cout<<  "HypoTestTool: Cast failed part 2" << std::endl;
          return false;
        }
...
//    doFit=true;
    ...
    { 
        ...
        RooFitResult * fitres = sbModel->GetPdf()->fitTo(*data,InitialHesse(false), Hesse(false),
                Minimizer(mMinimizerType.c_str(),"Migrad"), Strategy(2), Verbose(false),
        ...
        if (fitres->status() != 0) { 
            ...
            fitres = sbModel->GetPdf()->fitTo(*data,InitialHesse(true), Hesse(true),Minimizer(mMinimizerType.c_str(),"Migrad"), 
                    Strategy(3), PrintLevel(mPrintLevel+1), Constrain(constrainParams), Save(true), Verbose(false) );
        }
   ...
   //     toymcs->SetUseMultiGen(mOptimize);
    ...
    if (type == 1) { 
        ...
        ((AsymptoticCalculator*) m_hc)->SetQTilde(false); // bug in roostats. Need to turn on explicitly.
