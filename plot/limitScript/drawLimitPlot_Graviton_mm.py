...
outputtexfile=open("limitNumbers_Graviton_mm.tex", "w")
title='''
\\begin{table}
\\begin{center}
...
if (mode=='VBF'):
  hframe.GetYaxis().SetTitle("95% Limit on #sigma_{VBF} * BR(H#rightarrowZZ)[fb]")
else:
  hframe.GetYaxis().SetTitle("95% Limit on #sigma_{G*} #times BR(G* #rightarrow ZZ)[fb]")
...
leg.AddEntry(sm_pred, "#sigma_{G*} #times BR(G* #rightarrow ZZ)", "l")
leg.Draw()
...
AtlasUtil.DrawText(col_l, 0.71, "G* #rightarrow ZZ #rightarrow #mu#mu#nu#nu", size=0.037)
if (mode=='VBF'):
