...
//    return;
 ...
 // obs_nll = createNLL(data);//(RooNLLVar*)pdf->createNLL(*data);
  ...
  obs_nll = !doBlind ? createNLL(data) : NULL;//(RooNLLVar*)pdf->createNLL(*data);
  ...
  if(doInj)
  {
    RooDataSet* asimovData_inj = makeAsimovData(conditionalExpected, obs_nll, 0, NULL, NULL, -999, true, 1.);
