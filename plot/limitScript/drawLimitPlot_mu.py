...
for i in range(0,len(medians)):
    lim_obs.append(float(obsvalues[i]))
    lim_med.append(float(medians[i]))
...
hframe.GetXaxis().SetTitle("m_{H} [GeV/c^{2}]")
...
if (mode=='VBF'):
  ...
  hframe.GetYaxis().SetTitle("95% Limit on #sigma / #sigma(SM)")
...
leg.AddEntry(gr_obs, "Observed Median", "l")
...
AtlasUtil.DrawLuminosityFb(col_l, 0.78, 2, 13)
#AtlasUtil.DrawText(col_l, 0.71, "H#rightarrowZZ*#rightarrow4#font[12]{l}", size=0.037)
AtlasUtil.DrawText(col_l, 0.71, "H#rightarrowZZ*#rightarrow 2l2n", size=0.037)
if (mode=='VBF'):
    AtlasUtil.DrawText(col_l, 0.66, "VBF", size=0.037)
else:
    AtlasUtil.DrawText(col_l, 0.66, "ggF", size=0.037)

c.SaveAs("./limitplots/"+tag+"XSEC_NEW_MU.png")
c.SaveAs("./limitplots/"+tag+"XSEC_NEW_MU.eps")
c.SaveAs("./limitplots/"+tag+"XSEC_NEW_MU.root")

ofile = R.TFile("graphs_"+tag+"_mu.root", "RECREATE")
