...
datemark="".join(((datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')).split()[0]).split("-"))
...
fn_br = './CrossSections/BR_Graviton_ZZ_Temp13.txt'#'BR_emu.txt'
...
if (mode=="VBF"):
    ...
    xs = get_cxs("./CrossSections/XS_Graviton_Temp13.txt", masses)

xs_theo=get_cxs("./CrossSections/XS_Graviton_Temp13_theo.txt", masses)
...
for i in range(0,len(medians)):
    ...
    lim_obs.append(25*float(br[i])*float(xs[i])*obsvalues[i])
    lim_med.append(25*float(br[i])*float(xs[i])*medians[i])
    minus1sig.append(25*float(br[i])*float(xs[i])*minus1sigmas[i])
    minus2sig.append(25*float(br[i])*float(xs[i])*minus2sigmas[i])
    plus1sig.append(25*float(br[i])*float(xs[i])*plus1sigmas[i])
    plus2sig.append(25*float(br[i])*float(xs[i])*plus2sigmas[i])
#    sm.append(1000*float(br[i])*float(xs_theo[i]))
    sm.append(25*float(br[i])*float(xs[i]))

#    lim_obs.append(25*float(xs[i])*obsvalues[i]*1000/0.445)
#    lim_med.append(25*float(xs[i])*medians[i]*1000/0.445)
#    minus1sig.append(25*float(xs[i])*minus1sigmas[i]*1000/0.445)
#    minus2sig.append(25*float(xs[i])*minus2sigmas[i]*1000/0.445)
#    plus1sig.append(25*0.01*float(xs[i])*plus1sigmas[i]*1000/0.445)
#    plus2sig.append(25*0.01*float(xs[i])*plus2sigmas[i]*1000/0.445)
#    sm.append(float(br[i])*float(xs[i])*1000/0.445)
...
outputtexfile=open("limitNumbers_Graviton.tex", "w")
title='''
\\begin{table}
\\begin{center}
...
for i in range(0,len(medians)):
#    masspoint=i*100+300
    line="%s GeV  &  %4.2f  & %4.2f  & %4.2f  & %4.2f  & %4.2f   & %4.2f\\\\"%(str(masses[i]), lim_med[i], plus2sig[i], plus1sig[i], minus1sig[i], minus2sig[i], lim_obs[i])
...
for massi in xrange(len(masses)):
    masses[massi]=0.001*masses[massi]
...
hframe.GetXaxis().SetTitle("m_{G*} [TeV]")
#hframe.GetXaxis().SetRangeUser(300,1000)
#hframe.GetYaxis().SetTitle("95% Limit on #sigma/#sigma_{SM}")
if (mode=='VBF'):
  hframe.GetYaxis().SetTitle("95% Limit on #sigma_{VBF} * BF(H #rightarrow ZZ) [fb]")
else:
  hframe.GetYaxis().SetTitle("95% Limit on #sigma_{G*} #times BF(G* #rightarrow ZZ) [fb]")
...
leg.AddEntry(sm_pred, "#sigma_{G*} #times BF(G* #rightarrow ZZ)", "l")
leg.Draw()
...
AtlasUtil.DrawText(col_l, 0.71, "G* #rightarrow ZZ #rightarrow ll#nu#nu", size=0.037)
if (mode=='VBF'):
    AtlasUtil.DrawText(col_l, 0.66, "VBF", size=0.037)
else:
    AtlasUtil.DrawText(col_l, 0.66, "#kappa/#bar{M}_{PI}=1", size=0.037)

#c.SaveAs("./limitplots/"+tag+"XSEC_NEW_"+datemark+".png")
#c.SaveAs("./limitplots/"+tag+"XSEC_NEW_"+datemark+".eps")
#c.SaveAs("./limitplots/"+tag+"XSEC_NEW_"+datemark+".pdf")
#

c.SaveAs("./limitplots/"+tag+"XSEC_NEW.png")
c.SaveAs("./limitplots/"+tag+"XSEC_NEW.eps")
c.SaveAs("./limitplots/"+tag+"XSEC_NEW.pdf")
