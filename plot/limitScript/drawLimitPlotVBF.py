...
xs = get_cxs("./CrossSections/XS_VBF_new.txt", masses)
...
#
#    lim_obs.append(25*0.01*float(xs[i])*obsvalues[i]*1000/0.445)
#    lim_med.append(25*0.01*float(xs[i])*medians[i]*1000/0.445)
#    minus1sig.append(25*0.01*float(xs[i])*minus1sigmas[i]*1000/0.445)
#    minus2sig.append(25*0.01*float(xs[i])*minus2sigmas[i]*1000/0.445)
...
#
    lim_obs.append(25*float(xs[i])*obsvalues[i])
    lim_med.append(25*float(xs[i])*medians[i])
    minus1sig.append(25*float(xs[i])*minus1sigmas[i])
    minus2sig.append(25*float(xs[i])*minus2sigmas[i])
    plus1sig.append(25*float(xs[i])*plus1sigmas[i])
    plus2sig.append(25*float(xs[i])*plus2sigmas[i])
    sm.append(25*float(xs[i]))
#    lim_obs.append(obsvalues[i])
#    lim_med.append(medians[i])
#    minus1sig.append(minus1sigmas[i])
#    minus2sig.append(minus2sigmas[i])
#    plus1sig.append(plus1sigmas[i])
#    plus2sig.append(plus2sigmas[i])
#    sm.append(float(xs[i]))
...
outputtexfile=open("limitNumbersVBF.tex", "w")
title='''
\\begin{table}
\\begin{center}
...
if (mode=='VBF'):
  ...
  hframe.GetYaxis().SetTitle("95% Limit on #sigma_{ggF} * BR(H#rightarrowZZ)[fb]")
...
AtlasUtil.DrawLuminosityFb(col_l, 0.78, 5.8, 13)  ## in fb
#AtlasUtil.DrawText(col_l, 0.71, "H#rightarrowZZ*#rightarrow4#font[12]{l}", size=0.037)
AtlasUtil.DrawText(col_l, 0.71, "H#rightarrowZZ*#rightarrow 2l2#nu", size=0.037)
if (mode=='VBF'):
